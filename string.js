...
const verifySubstrs = (mainStr, head, body, tail) => {
  return mainStr.startsWith(head) && mainStr.endsWith(tail) && mainStr.includes(body)
...
function alternatingCaps(str) {
  return str
    .split('')
    .map((ele, index) => (index % 2 == 0 ? ele.toUpperCase() : ele.toLowerCase()))
    .join('');
}
 ...
 *               that takes a string and returns true if the PIN is valid and false if it's not.
 *
 ...
 *           validatePIN("") ➞ false
 *
 * Note - Some test cases contain special characters.
