 ...
 * The game starts with an integer, , that's used to build a  of  distinct integers in the inclusive range from 1 to n  (i.e., set = {1,2,3,4,......, n-1, n}).
...
function sillyGame(number) {
  let primeRange = number < temp[1] ? 0 : temp[0];

  for (let i = number < temp[1] ? 0 : temp[1]; i <= number; i++) {
    if (primeNumber(i)) {
      primeRange++;
    }
  }
...
console.log(sillyGame(92410));
console.log(sillyGame(92480));
console.log(sillyGame(97927));
console.log(sillyGame(98942));
console.log(sillyGame(96574));
console.log(sillyGame(97208));
console.log(sillyGame(90283));
...
for (let i = 0; i < arrInput.length; i++) {
  const output = sillyGame(arrInput[i]);
  if (arrOutput[i] !== output) {
    console.log('please check', output, arrOutput[i], arrInput[i]);
  }
}
